<template>
    <section class="md:w-[75%] mx-auto px-6 md:px-0 pg-title">
        <div class="">
            <h1 class="text-5xl pb-4 font-semibold">FAQ</h1>
            <h5 class="font-semibold text-[#96999B]">よくあるご質問</h5>
        </div>
    </section>
    ...
    <section class="xl:w-[75%] my-32 mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        <!-- LEFT: Category list -->
        <aside class="lg:col-span-4">
            <nav class="bg-[#F6F7F8] pb-16 px-3">
                <h5 class="font-bold mb-4 pb-4 px-3 pt-4 border-b border-gray-300 border-solid">よくあるご質問内容</h5>
                <ul class="space-y-2">
                    <li v-for="(cat, idx) in categories" :key="cat.id">
                        <button type="button" class="w-full flex items-center gap-3 px-3 py-3 rounded-lg transition
                         text-left focus:outline-none focus:ring-2 focus:ring-[#00B1E6]
                         hover:bg-white/70" :class="{
                            'bg-white shadow-sm ring-1 ring-black/5': activeId === cat.id
                        }" @click="setActive(cat.id)">
                            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded
                           text-xs font-semibold"
                                :class="activeId === cat.id ? 'bg-[#00B1E6] text-white' : 'bg-gray-200 text-gray-700'">
                                {{ idx + 1 }}
                            </span>
            ...
            <div class="space-y-8">
                <div v-for="(item, i) in activeFaqs" :key="i" class="border-b border-gray-200 pb-6">
                    <p class="font-bold mb-2">
                        <span class="text-[#00B1E6] text-sm">Q.</span>
                        {{ item.q }}
                    </p>
                    <p class="text-sm">
                ...
                <p v-if="activeFaqs.length === 0" class="text-gray-500">
                    質問はまだありません。
                </p>
            </div>
        </aside>
...
/**
 * Data model
 * - Each category has an id, title, and faqs array [{ q, a }]
 */
    ...
    },
    {
        ...
        faqs: [
            ...
            },
            {
                ...
                a: "A.	はい、可能です。エントリー後にご希望の日時をお伺いしますので、その際にご相談ください。"
            }
        ],
    },
    {
...
const activeId = ref(categories.value[0].id);
const activeFaqs = computed(() => categories.value.find(c => c.id === activeId.value)?.faqs ?? []);
